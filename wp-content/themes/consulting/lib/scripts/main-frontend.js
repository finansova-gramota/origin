...
jQuery(document).ready(function (){
	...
	jQuery(".logo-capital").click(function(){
		window.location = window.location.href + 'capital';
	});
	jQuery(".logo-career").click(function(){
		window.location = window.location.href + 'vozmozhnosti-kariery';
	});
	jQuery(".count-program").click(function(){
	...
	jQuery("#currency-switch").change(function(){
		...
		if(thisvalue == 'UAH (ГРАВЕ, МетЛайф)'){
			jQuery('#hryvna-option').show();
			jQuery('#dollar-option').hide();
		}
		if(thisvalue == 'USD (только ГРАВЕ)'){
			jQuery('#hryvna-option').hide();
			jQuery('#dollar-option').show();
		}
	});
	jQuery("#hryvna-option").change(function(){
		var thisvalue = jQuery(this).find("option:selected").text();
		if(thisvalue == "Другая сумма"){
			jQuery('#other-summa').show();			
		}
		if(thisvalue != "Другая сумма"){
			jQuery('#other-summa').hide();			
		}
	});
	jQuery("#menu-383-id").change(function(){
		var thisvalue = jQuery(this).find("option:selected").text();
		if(thisvalue == "Корпоративное групповое страхование (только для юр. лиц)"){
			jQuery('.label-cor-group').show();			
		}
		if(thisvalue != "Корпоративное групповое страхование (только для юр. лиц)"){
			jQuery('.label-cor-group').hide();			
		}
	});
	...
	jQuery("#to-visit-seminar").click(function() {
		var getUrl = window.location;
		var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
		window.location = baseUrl + '/posetit-seminar/';
	});
	jQuery("#menu-item-691").click(function() {
		var getUrl = window.location;
		var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
		window.location = baseUrl + '/capital/';
	});
