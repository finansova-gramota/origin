...
function consulting_thinkup_custom_logo() {
	...
	if ( ! empty( $check_logoset[0] ) ) {
	   if ( function_exists( 'the_custom_logo' ) ) {
		...
		$output .= '<span class="site-title-underline"></span>';
...
function consulting_thinkup_sidebar_css() {
...
$thinkup_woocommerce_layoutproduct = consulting_thinkup_var ( 'thinkup_woocommerce_layoutproduct' );
	...
	if ( is_front_page() ) {
		if ( $thinkup_homepage_layout == "option1" or empty( $thinkup_homepage_layout ) ) {		
			...
			wp_enqueue_style ( 'consulting-thinkup-sidebarleft' );
		} else if ( $thinkup_homepage_layout == "option3" ) {
			wp_enqueue_style ( 'consulting-thinkup-sidebarright' );
		}
	} else if ( is_page() ) {	
...
add_action( 'wp_enqueue_scripts', 'consulting_thinkup_sidebar_css', '11' );
