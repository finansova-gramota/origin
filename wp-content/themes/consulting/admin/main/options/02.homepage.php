...
<button class="logo-career">'. iconv(mb_detect_encoding("Узнать о карьере", mb_detect_order(), true), "UTF-8", "Узнать о карьере") .'</button></div>';
   ...
   // echo do_shortcode("[metaslider id=124]");
		...
		if(wp_is_mobile()){
			...
			echo '';
			echo "<div id='from-author-home'><p>".iconv(mb_detect_encoding("Для вас важно обеспечить финансовую защиту для себя и своей семьи? Рассчитайте необходимый капитал сейчас!
...
Ищете дополнительный доход, который может стать основным? Узнайте больше о карьере финансового консультанта!", mb_detect_order(), true)
					...
					, "UTF-8", "Для вас важно обеспечить финансовую защиту для себя и своей семьи? Рассчитайте необходимый капитал сейчас!
